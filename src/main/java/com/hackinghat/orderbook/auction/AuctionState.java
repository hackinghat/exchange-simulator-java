...
public class AuctionState implements Level1
{
    ...
    private static class AuctionRecord implements Comparable<AuctionRecord>
    {
        ...
        @Override
        public int compareTo(final AuctionRecord other)
        {
            return level.levelCompare(OrderSide.BUY, level);
        }

        private AuctionRecord(final Level level)
        {
            this.level = level;
            this.bidAggVolume = 0;
    ...
    long getMaximumVolume() { return maximumVolume; }
    ...
    {
        return auctionRecords.entrySet().stream()
                .filter(e -> e.getValue().getUncrossingVolume() == maximumVolume)
