...
public class MarketManager extends AbstractComponent implements Listener
{
    ...
    public double getLast() { return lastLevel.getPrice().doubleValue(); }

    Level getLastLevel() { return lastLevel; }
    ...
    public AuctionTriggerEvent priceMonitor(final Trade observed)
    {
        ...
        final double thisPrice = observed.getLevel().getPrice().doubleValue();
        final double lastPrice = getLastLevel().getPrice().doubleValue();
        // Check if the price has moved by more the threshold, we try to keep it numerically stable by
        // looking for a %age difference in excess of threshold + 1E-9
        final boolean auctionRequired = (Math.abs(1 - thisPrice / lastPrice) - priceMoveThreshold) > 1E-9;
...
//            dispatcher.dispatch(new AuctionTriggerEvent(this, timeMachine.toSimulationTime(), MarketState.AUCTION));
    ...
    /**
     * The time maching might have its run speed adjusted, that should require us to reschedule all the auction events
     * @param timeSpeedChangeEvent
     */
