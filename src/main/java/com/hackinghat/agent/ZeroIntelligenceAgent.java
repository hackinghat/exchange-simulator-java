...
import com.hackinghat.orderbook.InvalidMarketStateException;
...
public class ZeroIntelligenceAgent extends Agent {
    ...
    public static String getHeaders() {
        return "Name,Inspread,Buy,Market,Cancel";
    }
    ...
    boolean canAfford(final Order order, final Level1 marketTouch, final Level last) {
        switch (order.getSide()) {
            case BUY:
                float estimate = estimatePrice(order.getSide(), marketTouch, last);
                if (estimate == 0.f)
                    return false;
    ...
    private float estimatePrice(final OrderSide side, final Level1 level1, final Level reference) {
        final float best = level1.getPrice(side).getPrice();
        return best == 0.f ? (reference == null ? 0.f : reference.getPrice()) : best;
    }
