...
public class Level {
     ...
     * TODO: Don't use a rogue value to indicate a market level!
     */
    ...
    Level(final float price, final int level, final float tickSize) {
        if (level < 0)
            throw new IllegalArgumentException("Level must be greater than or equal to zero");
        if (price < 0)
            throw new IllegalArgumentException("Price must be greater than or equal to zero");
        if (tickSize <= 0.)
            throw new IllegalArgumentException("Ticksize should be greater than zero.");
        this.price = price;
        this.level = level;
        this.market = false;
     ...
     * @return true if this level is better than the otherLevel
     */
