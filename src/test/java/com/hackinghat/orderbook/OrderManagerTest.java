...
public class OrderManagerTest {
    ...
    private void checkTouch(float expectedBid, int expectedBidCount, long expectedBidVolume, float expectedOffer, int expectedOfferCount, long expectedOfferVolume, MarketState expectedState) {
    ...
    private void checkChanges(Agent buyer, double buyerCash, int buyerShares, Agent seller, double sellerCash, int sellerShares) {
        ...
        assertEquals("Seller shares", sellerShares, seller.sharesChange());
    }
    ...
    @SuppressWarnings("SameParameterValue")
    private Pair<Order, Order> setupTop(final float bidPrice, final float offerPrice, int quantity) {
    ...
    private Order submitLimit(Agent sender, OrderSide side, float price, final OrderManager orderManager, int... quantities) {
    ...
    private void makeBook(int nLevels, OrderSide side1, OrderSide side2, final Level startLimit, int quantity) {
        for (int i = 0; i < nLevels; ++i) {
            ...
            submitLimit(side2 == OrderSide.BUY ? buyer : seller, side2, VOD.worsenOnBook(startLimit, side2, i + 1).getPrice(), manager, quantity);
        }
        manager.process();
    }
    ...
    public void testPendingOrders() {
        ...
        manager.add(new Order(nextId(), OrderSide.BUY, VOD, 100.0f, 1000, buyer, timeMachine));
        manager.add(new Order(nextId(), OrderSide.BUY, VOD, 100.0f, 1000, buyer, timeMachine));
    ...
    public void testCancelInProcess() {
        Order b = new Order(nextId(), OrderSide.BUY, VOD, 100.0f, 500, nullAgent, timeMachine);
        manager.add(b);
        manager.process();
    ...
    @Test
    public void testClearTwoMarkets() {
    ...
    public void testClear() {
        try (final AgentImplTest buyAgent = new AgentImplTest(VOD, NotSoRandomSource.makeZeroRandom(), 3L);
            ...
            manager.add(sell1);
            manager.process();
            assertEquals(100.0, manager.getLevel1().getOffer().getLevel().getPrice(), 1E-6);
            ...
            manager.add(buy1);
            manager.process();
            Assert.assertEquals(100.0, manager.getLevel1().getOffer().getLevel().getPrice(), 1E-16);
    ...
    public void testMarket() {
        ...
        checkTouch(0.f, 4, 1000, 0.f, 4, 1000, MarketState.CHOICE);
        submitLimit(buyer, OrderSide.BUY, 150.0f, manager, 500);
        // The 150 got executed so now we've got ...
        checkTouch(0.f, 4, 1000, 0.f, 2, 500, MarketState.CHOICE);
        submitLimit(buyer, OrderSide.SELL, 150.0f, manager, 100);
        //TODO: The addition of the SELL 500@150.0, makes a market that allow the market orders to cross, is this correct?
        checkTouch(0.f, 3, 900, 0.f, 2, 500, MarketState.CHOICE);
    }
    ...
    public void testBackwardated() {
        ...
        Level midPrice = VOD.getLevel(150.0f);
        ...
        manager.process();
        checkTouch(0.f, 0, 0, 0.f, 0, 0, MarketState.CHOICE);
    }

    @Test
    public void testNormalMarketNoCrossing() {
        // Make a book where nothing crosses
        makeBook(10, OrderSide.BUY, OrderSide.SELL, VOD.getLevel(150.0f), 100);
        ...
        OrderInterestTest.checkInterest(OrderSide.BUY, buyVwap, 149.95f, 10, 1000);
        OrderInterestTest.checkInterest(OrderSide.SELL, sellVwap, 150.06f, 10, 1000);
        checkTouch(149.99f, 1, 100, 150.01f, 1, 100, MarketState.CONTINUOUS);
    }
    ...
    public void testUncrossedMarket() {
        ...
        submitLimit(buyer, OrderSide.BUY, 101.f, manager, 250);
        checkTouch(101.f, 2, 500, 0.0f, 0, 0, MarketState.CHOICE);
        ...
        checkTape(new int[]{250, 50}, new float[]{101.f, 101.f});
        checkNet(buyer, seller);
    }
    ...
    public void testLimitBuy() {
        setupTop(1.4f, 1.6f, 1000);
        submitLimit(buyer, OrderSide.BUY, 1.7f, manager, 500);
        checkNet(buyer, seller);
    ...
    public void testLimitSell() {
        ...
        submitLimit(seller, OrderSide.SELL, 1.3f, manager, 500);
        checkNet(buyer, seller);
        checkTape(new int[]{500}, new float[]{1.4f});
        checkChanges(buyer, -700.0, 500, seller, 700.0, -500);
    }
    ...
    public void testTooLateToCancel() {
        Pair<Order, Order> bidOffer = setupTop(1.4f, 1.6f, 1000);
        ...
        // the filled order
        manager.add(new Order(nextId(), OrderSide.SELL, VOD, 1.4f, 1000, seller, timeMachine));
        manager.add(bid);
        manager.process();
    }
    ...
    public void testStaleCancel() {
        ...
        // Now add a trade to match the top of the book, immediately followed by a trade to cancel
        // the filled order
        manager.add(new Order(nextId(), OrderSide.SELL, VOD, 1.4f, 500, seller, timeMachine));
        manager.add(bid);
        manager.process();
    }
    ...
    public void testRejectOnClosed() {
        ...
        try (final OrderManager closed = new OrderManager(marketManager, timeMachine, null, MarketState.CLOSED, LLOY, new SyncEventDispatcher(timeMachine), tape, null, Duration.ZERO)) {
            ...
            final Order rejected = submitLimit(buyer, OrderSide.BUY, 1.4f, closed, 1000);
    ...
    private void checkTape(final int[] tradeQuantities, final float[] tradePrices) {
    ...
    public void testAuctionUncross() throws AuctionException {
        ...
        try (final OrderManager auction = new OrderManager(marketManager, timeMachine, referenceLevel, MarketState.AUCTION, LLOY, new SyncEventDispatcher(timeMachine), tape, null, Duration.ZERO);
            ...
            checkInterest(auction, LLOY.getLevel(104.5f), 5200, 0);
        }
    }
    ...
    public void testPriceMonitoringAuctionImmediateCancel() throws Exception {
        ...
        try (final OrderManager auction = new OrderManager(marketManager, timeMachine, referenceLevel, MarketState.CONTINUOUS, LLOY, dispatcher, tape, null, Duration.ZERO)) {
            ...
            submitLimit(nullAgent, OrderSide.SELL, LLOY.getLevel(200.0f), auction, 200, 200);
            // The second order should be cancelled by the order manager
            checkTape(new int[]{200}, new float[]{200.f});
