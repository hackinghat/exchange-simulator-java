...
public class MarketManagerTest
{
    ...
    public void setUp()
    {
        ...
        referenceLevel = VOD.getLevel(100.0);
    ...
    public void testLastPriceAuction() throws Exception
    {
        ...
        assertEquals(referenceLevel, marketManager.getLastLevel());
        marketManager.priceMonitor(new Trade(this, "T2", VOD, null,null,  "O3", "O4", VOD.getLevel(referenceLevel.getPrice().doubleValue()+10.0), 100));
        assertEquals(VOD.getLevel(110.0d), marketManager.getLastLevel());
        assertEquals(0, auctionEventReceiver.events.size());
        assertFalse(marketManager.isAuctionInProgress());
        marketManager.priceMonitor(new Trade(this, "T3", VOD,null,null, "O5", "O6", VOD.getLevel(referenceLevel.getPrice().doubleValue()-2.0), 100));
        // The first auction trigger event will be published by the market manager and so there will be no dispatch
        assertEquals(0, auctionEventReceiver.events.size());
        assertEquals(VOD.getLevel(98.0d), marketManager.getLastLevel());
    ...
    public void testNotifyTradeDuringAuction()
    {
        ...
        marketManager.priceMonitor(new Trade(this, "T5", VOD, timeMachine.toSimulationTime(), null, "O5", "O6", VOD.getLevel(lastPx-20.0), 100));
        assertTrue(marketManager.isAuctionInProgress());
        // This one is illegal
        marketManager.priceMonitor(new Trade(this, "T6", VOD, timeMachine.toSimulationTime(),  null, "O5", "O6", VOD.getLevel(lastPx-20.0), 100));
    }
